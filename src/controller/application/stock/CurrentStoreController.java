...
public class CurrentStoreController implements Initializable {
    ...
    @FXML
    private ComboBox<String> cbSoteViewSupplier;
    @FXML
    private ComboBox<String> cbSoteViewBrands;
    @FXML
    private ComboBox<String> cbSoteViewCategory;
    @FXML
    ...
    @FXML
    private TableColumn<Object, Object> tblClmProductPurchasePrice;
    @FXML
    ...
    private void tfSearchOnKeyRelese(KeyEvent event) {
        ...
        currentProductGateway.searchView(productCurrent);

    }

    @FXML
    private void cbSoteViewSupplierOnClick(MouseEvent event) {

        con = dbCon.geConnection();
        cbSoteViewSupplier.getItems().clear();
        cbSoteViewBrands.setPromptText("Select Brand");
        cbSoteViewCategory.setPromptText("Select Category");
        try {
            pst = con.prepareStatement("select * from Supplier");
            rs = pst.executeQuery();
            while (rs.next()) {
                cbSoteViewSupplier.getItems().remove(rs.getString(2));
                cbSoteViewSupplier.getItems().add(rs.getString(2));
            }
            rs.close();
            con.close();
            pst.close();
    ...
    private void cbSoteViewBrandOnClick(MouseEvent event) {
        ...
        suplyerName = cbSoteViewSupplier.getSelectionModel().getSelectedItem();
        suplyerId = sql.getIdNo(suplyerName, suplyerId, "Supplier", "SupplierName");

        try {
            pst = con.prepareStatement("select * from Brands where SupplierId=?");
    ...
    @FXML
    private void cbSoteViewCategoryOnClick(MouseEvent event) {

        con = dbCon.geConnection();
        cbSoteViewCategory.getItems().clear();
        suplyerName = cbSoteViewSupplier.getSelectionModel().getSelectedItem();
        suplyerId = sql.getIdNo(suplyerName, suplyerId, "Supplier", "SupplierName");
        brandId = sql.getBrandID(suplyerId, brandId, brandName);
        try {
            pst = con.prepareStatement("select * from Category where SupplierId=? and BrandId=?");
            pst.setString(1, suplyerId);
            pst.setString(2, brandId);
            rs = pst.executeQuery();
            while (rs.next()) {
                cbSoteViewCategory.getItems().add(rs.getString(2));
            }
            rs.close();
            con.close();
            pst.close();
    ...
    private void btnDeleteOnAction(ActionEvent event) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            currentProductSetup.delete(productCurrent);
    ...
    public void viewDetails() {
        ...
        tblClmProductPurchasePrice.setCellValueFactory(new PropertyValueFactory<>("purchasePrice"));
        ...
        currentProductGateway.viewFistTen(productCurrent);
    ...
    @FXML
    private void cbSoteViewSupplierOnAction(ActionEvent event) {

        if (cbSoteViewSupplier.getSelectionModel().getSelectedItem() == null) {

        } else {
            suplyerName = cbSoteViewSupplier.getSelectionModel().getSelectedItem();
            productCurrent.supplierName = suplyerName;
            currentProductGateway.searchBySupplier(productCurrent);
        }
    ...
    private void cbSoteViewBrandOnAction(ActionEvent event) {
        ...
        if (cbSoteViewBrands.getSelectionModel().getSelectedItem() == null) {
            ...
            suplyerName = cbSoteViewSupplier.getPromptText();
            productCurrent.supplierName = suplyerName;
            productCurrent.brandName = brandName;
            currentProductGateway.searchByBrand(productCurrent);
        }

    }

    @FXML
    private void cbSoteViewCategoryOnAction(ActionEvent event) {

        if (cbSoteViewCategory.getSelectionModel().getSelectedItem() == null) {

        } else {
            brandName = cbSoteViewBrands.getSelectionModel().getSelectedItem();
            suplyerName = cbSoteViewSupplier.getPromptText();
            categoryName = cbSoteViewCategory.getSelectionModel().getSelectedItem();
            productCurrent.supplierName = suplyerName;
            productCurrent.brandName = brandName;
            productCurrent.categoryName = categoryName;
            currentProductGateway.searchByCategory(productCurrent);
        }
    ...
    private void btnRefreshOnACtion(ActionEvent event) {
        ...
        cbSoteViewBrands.getItems().clear();
        cbSoteViewCategory.getItems().clear();
        cbSoteViewSupplier.setPromptText("Select supplier");
        ...
        tblClmProductAddBy.setCellValueFactory(new PropertyValueFactory<>("user"));
        tblClmProductdate.setCellValueFactory(new PropertyValueFactory<>("date"));
        currentProductGateway.view(productCurrent);
    ...
    private void cbSoteViewRMAOnAction(ActionEvent event) {
        ...
        currentProductGateway.searchByRMA(productCurrent);
